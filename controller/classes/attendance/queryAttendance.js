...
const queryAttendance = async (req, res) => {
    try {
        ...
        const [results] = await db.execute(`SELECT * FROM attendance as a LEFT JOIN student as s ON a.id_student = s.id WHERE LOWER(s.name) LIKE LOWER('%${student}%') AND a.id_class = '?'`,
        [classes]);
