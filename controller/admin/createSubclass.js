...
const createSubClass = async (req, res) => {
  try {
    ...
    if (similar.length > 0) {
      return res.send({
        message: `Subclass / Jenis for ${subClass} already exists! Try different name?`,
      });
    }
    ...
    await db.execute(
      'INSERT INTO jenis (jenis, keterangan, createdBy) VALUES (?,?,?)',
      [subClass, keterangan || null, email],
    );

    res.send({
      message: `subClass / Jenis Kelas ${subClass} has been created!`,
    });
  } catch (error) {
    console.log(error);
    res.status(500);
